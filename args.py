...
def get_args():
    ...
    parser.add_argument('--loss_list','--list', nargs='+', help='<Required> Set flag', required=True)
    parser.add_argument('--query_list', nargs='+', help='<Required> Set flag', required=True)
    parser.add_argument(
        "--ff_dim",
        type=int,
        default=2048,
    ...
    parser.add_argument(
        "--flow_id", type=str, default="v1", help="second demo model"
    )
    parser.add_argument(
        "--vis_path", type=str, default="", help="second demo model"
    )
    parser.add_argument(
        "--save_dir", type=str, default="./save_models/", help="path to save dir"
    )
    parser.add_argument('--remove_bb', action='store_true')
    parser.add_argument('--remove_mot', action='store_true')
    parser.add_argument('--clean', action='store_true')
    parser.add_argument('--clean_linear', action='store_true')
    parser.add_argument(
        "--fusion", type=str, default="pecasa", help="second demo model"
    )
    parser.add_argument(
        "--mc", type=int, default=5, help="number of multiple choices"
    )
    parser.add_argument(
        "--sr", type=int, default=2, help="number of multiple choices"
    )
    parser.add_argument(
    ...
    if args.dataset not in ["howto100m", "howtovqa"]:  # VideoQA dataset
        ...
        # args.train_csv_path = os.path.join(load_path, "time2", "train.csv")
        # if args.dataset == 'tgifqa':
        #     args.val_csv_path = os.path.join(load_path, "time2", "test.csv")
        # else:
        #     args.val_csv_path = os.path.join(load_path, "time2", "val.csv")
        # args.test_csv_path = os.path.join(load_path, "time2", "test.csv")
