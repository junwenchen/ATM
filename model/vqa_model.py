...
class Embeddings(nn.Module):
    def __init__(
        self, d_model, language_len, vision_len, dropout, sinusoidal_pos_embds, d_pos=128, clean_layer=False
    ):
        super().__init__()
...
class ATM(nn.Module):
    def __init__(
        ...
        self.position_v = Embeddings(d_model, 20, 8, dropout, True, d_pos, self.args.clean_linear)
        #####################hie position###################
    ...
    # def get_vqa_embedding_rfcpos(self, video):

        video_o, video_f, video_m = video[0], video[1], video[2]
        # video_o, video_f = video[0], video[1] #, video[2]
        video_f = self.linear_video(video_f)
        video_f = gelu(video_f)
        video_f = self.norm_video(video_f) #(bs, numc, numf, dmodel)

        video_m = self.linear_video_mot(video_m)
        video_m = gelu(video_m)
        video_m = self.norm_video_mot(video_m)
        
        bsize, numc, numf, numr, fdim = video_o.size()
        numc_f = video_f.shape[1]
        ...
        # X = X.view(bsize, numc, numf, numr, -1).view(bsize*numc, numf, numr, -1)
        ...
        X_o = self.gnn(X, A)
        X_o += X
        # X_o = X.reshape(bsize*numc*numf, numr, -1)
        
        satt = self.satt_pool(X_o)
        X_o = torch.sum(X_o*satt, dim=-2)

        X_o = X_o.view(bsize, numc, numf, -1).mean(dim=2)


        if X_o.shape[1]*2 == video_f.shape[1]:
            X_o = X_o.unsqueeze(2).repeat(1,1,2,1).view(bsize,video_f.shape[1],video_f.shape[2])
        
        if self.remove_bb:
            video = self.merge_fr(video_f)
        else:
            video = self.merge_fr(torch.cat([video_f, X_o], dim=-1))

        if not self.remove_mot:
            video = self.merge_frm(torch.cat([video, video_m], dim=-1))
        return video, numc_f, ans_n
    ...
    def forward(
        ...
        if mode == "vqa":
            ...
            qverb_g, qverb_w = (
                self.get_answer_embedding(q_verb)
                if answer is not None
                else self.answer_embeddings
            )
            averb_g, averb_w = (
                self.get_answer_embedding(a_verb)
                if answer is not None
                else self.answer_embeddings
            )
            if self.Q != 0:
                ...
                video, numc_f, ans_n = self.get_vqa_embedding_rfcpos(video, answer_w, seq_len)
                # video_proj = self.get_vqa_embedding_rfcpos(video, answer_w, seq_len)
                feat_shuffle = video[:, torch.randperm(video.size()[1]), :]

                video_proj = self.position_v(video)
                if self.args.fusion == "pecasa":
                    video_proj = self.cm_interaction(video_proj, numc_f, answer_w, seq_len, ans_n)
                    attended_v = self.mmt(x=video_proj, attn_mask=video_mask)[0]
                elif self.args.fusion == "pesaca":
                    attended_v = self.mmt(x=video_proj, attn_mask=video_mask)[0]
                elif self.args.fusion == "peca":
                    attended_v = self.cm_interaction(video_proj, numc_f, answer_w, seq_len, ans_n)
                global_feat = attended_v.mean(dim=1)
                fusion_proj = self.vqproj(global_feat)


                shuffle_video_proj = self.position_v(feat_shuffle)
                shuffle_attended_v = self.mmt(x=shuffle_video_proj, attn_mask=video_mask)[0]
                shuffle_global_feat = shuffle_attended_v.mean(dim=1)
                shuffle_fusion_proj = self.vqproj(shuffle_global_feat)
                

            if fusion_proj is not None and answer_g.device != fusion_proj.device:
                answer_g = answer_g.to(fusion_proj.device)
            if answer is not None:
                return fusion_proj, shuffle_fusion_proj, answer_g, qverb_g, averb_g
            else:
